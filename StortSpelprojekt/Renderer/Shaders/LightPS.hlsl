...
cbuffer matrixBufferLightPass : register(b3)
{
	matrix lightView;
	matrix lightProj;
}
...
float4 main(VS_OUT input) : SV_TARGET
{
	...
	if (howMuchLight > 0.0f  && l < s.range)
	{
		...
		else if (dot(-normalize(lightToPixel), normalize(s.direction)) > 0.95f)
		{
			finalColor += s.color * s.intensity;
			finalColor *= dot(-normalize(lightToPixel), s.direction);
		}
	}
