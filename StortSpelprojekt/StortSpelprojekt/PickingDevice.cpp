...
{
	Box pickBox = Box(std::abs(points[0]._x - points[3]._x),
		1.0f,
...
vector<GameObject*> PickingDevice::boxPickObjects(POINT mousePoint, vector<GameObject*> pickableObjects)
{
	...
	if (legitForReal)
	{
		Box pickBox = createPickBox(points); 

		for (unsigned int i = 0; i < pickableObjects.size(); i++)
		{
			Box pickObject = Box(1.0f, 3.0f, 1.0f, Vec3(pickableObjects[i]->GetPosition()), Vec3(pickableObjects[i]->GetRotation()));

			if (Collision(pickBox, pickObject))
			{
				pickedObjects.push_back(pickableObjects[i]);
			}
		}
